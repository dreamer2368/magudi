...
program spatial_inner_product
  ...
  logical :: lookForInput = .false., lookForOutput = .false., lookForMollifier = .false.,                   &
              inputFlag = .false., outputFlag = .false., mollifierFlag = .false., adjointFlag = .false.
  ...
  if (adjointFlag) then
    write(message, '(A)') "Adjoint vectors will be loaded."
    call writeAndFlush(MPI_COMM_WORLD, output_unit, message)
  end if
  ...
  do i=1,2
    if(adjointFlag) then
      call region%loadData(QOI_ADJOINT_STATE, Qfilenames(i))
    else
    ...
    do j=1,size(region%states)
      ...
      if(adjointFlag) then
        temp(j)%F = region%states(j)%adjointVariables
      else
