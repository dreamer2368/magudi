...
program q_criterion
  ...
  interface
     ...
     subroutine saveQCriterion(region, filename)
  ...
  if (command_argument_count() >= 1) then !... only one solution file to process.
     ...
     if (filename(i-1:i) == ".q") then
        filename = filename(:i-2) // ".q_criterion.f"
     else
        filename = PROJECT_NAME // ".q_criterion.f"
     end if
     call saveQCriterion(region, filename)

  else

     call getRequiredOption("q_criterion/save_interval", saveInterval)
     call getRequiredOption("q_criterion/start_timestep", startTimestep)
     call getRequiredOption("q_criterion/end_timestep", endTimestep)
     ...
     do i = startTimestep, endTimestep, saveInterval
        ...
        write(filename, '(2A,I8.8,A)') trim(outputPrefix), "-", i, ".q_criterion.f"
        call saveQCriterion(region, filename)
...
subroutine saveQCriterion(region, filename)
  ...
  ! <<< Internal modules >>>
  use CNSHelper, only : computeDependentVariables, computeQCriterion
  ...
  ! <<< Local variables >>>
  type :: t_QCriterionInternal
     SCALAR_TYPE, pointer :: buffer(:,:) => null()
  end type t_QCriterionInternal
  type(t_QCriterionInternal), allocatable, save :: data_(:)
  ...
  do i = 1, size(region%states)
     ...
     call computeQCriterion(nDimensions,                                                     &
          region%states(i)%velocityGradient, qCriterion = data_(i)%buffer(:,1))
