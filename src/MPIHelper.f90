...
contains
  ...
  subroutine pigeonhole(nPigeons, nHoles, holeIndex, offset, nPigeonsInHole)
    ...
    !> pigeon. The number of pigeons in the first `holeIndex` holes (zero if `holeIndex=0`),
    !> and the number of pigeons in the hole with zero-based index `holeIndex` are returned as
    !> `offset` and `nPigeonsInHole`, respectively.
  ...
  subroutine fillGhostPoints(comm, arrayWithGhostPoints,                                     &
    ...
    !> direction `direction`, respectively. Periodicity information is obtained from the MPI
    !> communicator `comm`, which is assumed to contain Cartesian topology information. If the
    !> direction `direction` is periodic and the optional argument `periodicOffset` is
    !> present, the first `periodicOffset(2)` and last `periodicOffset(1)` ``physical'' points
    !> are skipped when sending data (this is useful for a ``ring'' topology along direction
    !> `direction` with overlapping points).
    ...
    real(SCALAR_KIND), intent(inout) :: arrayWithGhostPoints(:,:,:,:)
    ...
    real(SCALAR_KIND), allocatable :: receivedFromPreviousProcess(:,:,:),                    &
    ...
    call MPI_Cart_get(comm, nDimensions, processDistribution, isPeriodic,                    &
         processCoordinates, ierror)
    if (processDistribution(direction) == 1 .and.                                            &
         numGhostPoints(1) /= numGhostPoints(2)) return
    ...
    tagPrev = 1
    tagNext = 2
    call MPI_Cart_shift(comm, direction - 1, 1, rankOfPreviousProcess,                       &
         rankOfNextProcess, ierror)
    ...
    if (numGhostPoints(1) > 0) then
       ...
       call MPI_Irecv(receivedFromPreviousProcess, size(receivedFromPreviousProcess),        &
            SCALAR_TYPE_MPI, rankOfPreviousProcess,                                          &
            tagNext, comm, request(1), ierror)
    ...
    if (numGhostPoints(2) > 0) then
       ...
       call MPI_Irecv(receivedFromNextProcess, size(receivedFromNextProcess),                &
            SCALAR_TYPE_MPI, rankOfNextProcess, tagPrev, comm, request(2), ierror)
  ...
  subroutine gatherAlongDirection(cartesianCommunicator, localArray,                         &
    ...
    !> Gathers a grid vector `localArray` from all processes along direction `direction` and
    !> places them sequentially in the grid vector `gatheredArray`. The first dimension of
    !> `localArray` and `gatheredArray` are flattened from three-dimensional shapes using
    !> Fortran-ordering. `localSize` is the ``original'' shape of `localArray` prior to
    !> flattening. The ``original'' shape of `gatheredArray` is identical to `localArray`
    !> except along dimension `direction`, where it is the sum of `localSize(direction)` taken
    !> across all processes along direction `direction`. `offsetAlongDirection` is the
    !> zero-based index of the first element of `gatheredArray` along dimension `direction`
    !> received from the ``current'' process.
    ...
    real(SCALAR_KIND), intent(out) :: gatheredArray(:,:)
    ...
    real(SCALAR_KIND), allocatable :: sendBuffer(:,:), receiveBuffer(:,:)
