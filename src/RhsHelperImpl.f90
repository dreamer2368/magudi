...
contains
  ...
  ! subroutine addFarFieldLinearizedPenalty(simulationFlags, solverOptions,                       &
  !      grid, state, patchFactories)
  !
  !   ! <<< External modules >>>
  !   use MPI
  ...
  !        direction, gridIndex, patchIndex, ierror
  !   SCALAR_TYPE, allocatable :: localConservedVariables(:), localVelocity(:),                &
  !        localDeltaVelocity(:), localMetricsAlongDirection1(:),                              &
  !        localMetricsAlongDirection2(:), localViscousFluxJacobian(:,:), temp1(:,:,:), temp2(:,:)
  !   class(t_Patch), pointer :: patch => null()
  !
  !   farFieldPatchesExist = queryPatchTypeExists(patchFactories,                              &
  !        'SAT_FAR_FIELD', grid%index)
  ...
  !   temp1 = 0.0_wp
  !
  !   allocate(localDeltaVelocity(nDimensions))
  !   allocate(localVelocity(nDimensions))
  !
  !   if (allocated(patchFactories)) then
  !      do iPatchFactory = 1, size(patchFactories)
  ...
  !                     localVelocity = state%velocity(gridIndex,:)
  !                     localDeltaVelocity =                                                   &
  !                             - localVelocity * state%adjointVariables(gridIndex,1)          &
  !                             + state%adjointVariables(gridIndex,2:nDimensions+1)
  !                     temp1(gridIndex,1:nDimensions,1) = temp1(gridIndex,1:nDimensions,1)    &
  !                                                         + localDeltaVelocity
  !
  !                     temp1(gridIndex,nUnknowns-1,1) = temp1(gridIndex,nUnknowns-1,1)        &
  !                                       - state%specificVolume(gridIndex,1)                  &
  !                                         * state%conservedVariables(gridIndex,nUnknowns)    &
  !                                         * state%adjointVariables(gridIndex,1)              &
  !                                       - sum(localVelocity * localDeltaVelocity)            &
  !                                       + state%adjointVariables(gridIndex,nUnknowns)
  !
  !                  end do !... i = patch%offset(1) + 1, patch%offset(1) + patch%localSize(1)
  !               end do !... j = patch%offset(2) + 1, patch%offset(2) + patch%localSize(2)
  !            end do !... k = patch%offset(3) + 1, patch%offset(3) + patch%localSize(3)
  ...
  !      end do !... iPatchFactory = 1, size(patchFactories)
  !
  !      temp1(:,nUnknowns-1,1) = temp1(:,nUnknowns-1,1) * solverOptions%ratioOfSpecificHeats
  !      do l = 1, nUnknowns-1
  !        temp1(:,l,1) = temp1(:,l,1) * state%specificVolume(:,1)
  !      end do
  !      do l = 2, nDimensions
  !        temp1(:,:,l) = temp1(:,:,1)
  !      end do
  !      do l = 1, nDimensions
  !        call grid%firstDerivative(l)%apply(temp1(:,:,l), grid%localSize)
  !      end do
  !   end if !... allocated(patchFactories)
  !
  !   allocate(localConservedVariables(nUnknowns))
  ...
  !   SAFE_DEALLOCATE(localConservedVariables)
  !
  ! end subroutine addFarFieldLinearizedPenalty
