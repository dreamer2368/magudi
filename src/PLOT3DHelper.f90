...
module PLOT3DHelper
  ...
  private :: swapEndianness, swapIntegerEndianness_, swapScalarEndianness_,                  &
       plot3dGetOffsetFromGridSizes_, plot3dGetOffsetFromFile_
...
contains
  ...
  subroutine plot3dWriteSingleGrid(comm, filename, offset, mpiDerivedTypeScalarSubarray,     &
    ...
    call MPI_File_set_view(mpiFileHandle, offset, MPI_INTEGER,                               &
         mpiDerivedTypeIntegerSubarray, "native", MPI_INFO_NULL, ierror)
  ...
  subroutine plot3dWriteSingleSolutionHeader(comm,                                           &
       filename, offset, solutionHeader, success)

    !> Writes the auxiliary solution data (consisting of 4 real(SCALAR_KIND) values)
    ...
    character(len = *), intent(in) :: filename
    integer(kind = MPI_OFFSET_KIND), intent(inout) :: offset
    real(SCALAR_KIND), intent(in) :: solutionHeader(4)
    logical, intent(out) :: success
  ...
  end subroutine plot3dWriteSingleSolutionHeader
  ...
  subroutine plot3dReadSingleGrid(comm, filename, offset, mpiDerivedTypeScalarSubarray,      &
    ...
    !> `plot3dGetOffset`. If IBLANK values are not present in `filename`, `iblank` is set to
    !> 1.
  ...
  subroutine plot3dReadSingleSolutionHeader(comm, filename,                                  &
       offset, solutionHeader, success)

    !> Reads the auxiliary solution data (consisting of 4 real(SCALAR_KIND) values)
    !> `solutionHeader` corresponding to a single block at offset `offset` from the
    ...
    character(len = *), intent(in) :: filename
    integer(kind = MPI_OFFSET_KIND), intent(inout) :: offset
    real(SCALAR_KIND), intent(out) :: solutionHeader(4)
    logical, intent(out) :: success
    ...
    if (.not. descriptor%isEndiannessNative) then
       do i = 1, 4
          call swapEndianness(solutionHeader(i))
       end do
    end if
    offset = offset + 4 * SIZEOF_SCALAR + 2 * SIZEOF_PLOT3D_OFF
  ...
  end subroutine plot3dReadSingleSolutionHeader
