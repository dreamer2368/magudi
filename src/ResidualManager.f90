...
contains
  ...
  subroutine setup(this, prefix, region)
    ...
    do i = 1, size(this%tolerances)
       this%tolerances(i) = getOption(trim(prefix) // "/convergence_limit",                  &
            this%tolerances(i))
    end do
  ...
  subroutine compute(this, region)
    ...
    call MPI_Allreduce(MPI_IN_PLACE, this%residuals, size(this%residuals),                   &
         SCALAR_TYPE_MPI, MPI_MAX, region%comm, ierror)
