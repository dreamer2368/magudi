...
module ErrorHandler
  ...
  integer, private :: mpiWindowBase = 0, mpiWindow = MPI_WIN_NULL
...
contains
  ...
  subroutine gracefulExit(comm, errorMessage)
    ...
    !> Attempts to gracefully exit from an MPI application after an error has occurred.
    !> Aborts execution from all processes in `comm` if its relationship with `MPI_COMM_WORLD`
    !> is `MPI_UNEQUAL`. This subroutine must be called collectively by all processes in
    !> `comm`.
