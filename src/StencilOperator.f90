...
contains
  ...
  subroutine setupOperator(this, stencilScheme)
    ...
    if (trim(stencilScheme) == "SBP 1-2 first derivative") then
       ...
       this%rhsBoundary1(1:7,2) = (/ 0.085777408970_wp, 0.722371828476_wp,                   &
            0.356848072173_wp, -0.223119093072_wp, 0.057347064865_wp,                        &
            0.000747264596_wp, 0.000027453993_wp /)

       this%rhsBoundary1(1:7,3) = (/ -0.032649010764_wp, 0.143339502575_wp,                  &
            0.72667882202_wp, 0.294622121167_wp, -0.186711738069_wp,                         &
            0.062038376258_wp, -0.007318073189_wp /)
  ...
  pure subroutine applyOperatorAtInteriorPoints(this, xWithGhostPoints, x, gridSize)
    ...
    ! <<< Arguments >>>
    class(t_StencilOperator), intent(in) :: this
    real(SCALAR_KIND), intent(in) :: xWithGhostPoints(:,:,:,:)
    real(SCALAR_KIND), intent(out) :: x(:,:)
    integer, intent(in) :: gridSize(3)
  ...
  pure subroutine projectOnBoundaryAndApplyOperator_1(this, x, gridSize, faceOrientation)
    ...
    integer, parameter :: wp = SCALAR_KIND
    integer :: i, j, k, l, m, n
    real(SCALAR_KIND), allocatable :: x_(:,:,:,:)
