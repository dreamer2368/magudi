...
module Grid_mod
  ...
  type, public :: t_Grid
     ...
     real(SCALAR_KIND), dimension(:,:), allocatable :: coordinates, jacobian, metrics, norm, &
...
contains
  ...
  subroutine setup(this, index, globalSize, comm, processDistribution,                       &
  ...
  subroutine loadData(this, quantityOfInterest, filename, offsetInBytes, success)
  ...
  subroutine saveData(this, quantityOfInterest, filename, offsetInBytes, success)
    ...
    do i = 1, this%nDimensions
       ...
       if (allocated(this%adjointFirstDerivative)) then
          if (simulationFlags_%useContinuousAdjoint .or. trim(val) == "null matrix") then
             ...
             this%adjointFirstDerivative(i)%rhsInterior =                                    &
                  -this%firstDerivative(i)%rhsInterior
             this%adjointFirstDerivative(i)%rhsBoundary1 =                                   &
                  -this%firstDerivative(i)%rhsBoundary1
             this%adjointFirstDerivative(i)%rhsBoundary2 =                                   &
                  -this%firstDerivative(i)%rhsBoundary2
          else
       ...
       if (allocated(this%secondDerivative)) then
          ...
          call this%secondDerivative(i)%update(this%comm, i,                                 &
               this%periodicityType(i) == OVERLAP)
       end if
  ...
  subroutine computeCoordinateDerivatives(this, direction, coordinateDerivatives)
    ...
    real(SCALAR_KIND), intent(out) :: coordinateDerivatives(:,:)
    ...
    logical :: isPeriodic(3)
    real(SCALAR_KIND), allocatable :: xWithGhostPoints(:,:,:,:)
  ...
  subroutine update(this, hasNegativeJacobian, errorMessage)
    ...
    real(SCALAR_KIND), allocatable :: jacobianMatrixInverse(:,:), F(:,:)
    logical :: hasNegativeJacobian_
    character(len = STRING_LENGTH) :: message
    real(SCALAR_KIND) :: jacobianOutsideRange
  ...
  subroutine computeGradientOfScalar(this, f, gradF)
    ...
    ! <<< Arguments >>>
    class(t_Grid) :: this
    real(SCALAR_KIND), intent(in) :: f(:)
    real(SCALAR_KIND), intent(out) :: gradF(:,:)
  ...
  subroutine findMinimum(this, f, fMin, iMin, jMin, kMin)
    ...
    class(t_Grid) :: this
    real(SCALAR_KIND), intent(in) :: f(:)
    real(SCALAR_KIND), intent(out) :: fMin
  ...
  subroutine findMaximum(this, f, fMax, iMax, jMax, kMax)
    ...
    class(t_Grid) :: this
    real(SCALAR_KIND), intent(in) :: f(:)
    real(SCALAR_KIND), intent(out) :: fMax
  ...
  function isVariableWithinRange(this, f, fOutsideRange,                                     &
    ...
    class(t_Grid) :: this
    real(SCALAR_KIND), intent(in) :: f(:)
    real(SCALAR_KIND), intent(out) :: fOutsideRange
  ...
  subroutine applyFilter(this, f, timestep)
    ...
    ! <<< Arguments >>>
    class(t_Grid) :: this
    real(SCALAR_KIND), intent(inout) :: f(:,:)
