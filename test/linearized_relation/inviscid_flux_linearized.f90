...
program inviscid_flux_linearized
...
end program inviscid_flux_linearized
...
subroutine testLinearizedRelation(identifier, nDimensions, success, isPeriodic, tolerance)
  ...
  ! Compute linearized rhs for inviscid flux
  ...
  temp1 = 0.0_wp
  ! (1) Compute jacobian of inviscid flux dotted with delta variables
  ...
  do k = 1, size(stepSizes)
    ...
    ! write(errorMessage,'(E8.3,3(3X,E32.15))') stepSizes(k), scalar1,                         &
    !                                       scalar2/stepSizes(k), errorHistory(k)
