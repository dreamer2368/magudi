...
program artificial_dissipation
...
end program artificial_dissipation
...
subroutine testAdjointRelation(identifier, nDimensions, success, isPeriodic, tolerance)
  ...
  ! (1) add dissipation
  if (simulationFlags%dissipationOn)                                                         &
       call addDissipation(ADJOINT, simulationFlags, solverOptions, grid, state0)
  ...
  do k = 1, size(stepSizes)
    ...
    errorHistory(k) = abs( (scalar2/stepSizes(k) + scalar1)/scalar1 )   &
                                                  *stepSizes(k) !check absolute error for linear operators
  ...
  elseif (k > 3) then
     ...
     if (.not. success) print *, 'error history: ', errorHistory(1:k)
  else
     success = .false.
     print *, 'error history: ', errorHistory(1:k)
  end if
