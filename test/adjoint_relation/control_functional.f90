...
subroutine testAdjointRelation(nDimensions, success, isPeriodic, tolerance)
  ...
  real(wp) :: timeStepSize, time, startTime, targetState
  ...
  region%states(1)%targetState = random(-10.0_wp, 10.0_wp)
  targetState = region%states(1)%targetState(1,1)
  ...
  ! randomize timesteps
  nTimesteps = random(10,20)
  saveInterval = random(2,3)
  ...
  do k = 1, size(stepSizes)
    ! forward initial condition: uniform over space, unknowns
    region%states(1)%conservedVariables = forwardState(1,1)
    region%states(1)%targetState = targetState
