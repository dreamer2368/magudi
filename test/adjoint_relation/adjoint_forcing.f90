...
program full_rhs
  ...
  interface
     ...
     subroutine testAdjointRelation(solver,region,success,tolerance)
       use Region_mod, only : t_Region
       use Solver_mod, only : t_Solver
  ...
  ! Main code logic.
  success = .true.
  call testAdjointRelation(solver,region,success)

  if ( success ) then
    write(message, '(A)') "Cost target adjoint forcing test is passed."
    call writeAndFlush(MPI_COMM_WORLD, output_unit, message)
  else
    write(message, '(A)') "Cost target adjoint forcing test failed."
    call writeAndFlush(MPI_COMM_WORLD, output_unit, message)
    stop -1
  end if
...
subroutine testAdjointRelation(solver,region,success,tolerance)
  ...
  ! Compute the baseline cost functional.
  J0 = functional%compute(region)

  ! Compute adjoint forcing from cost functional.
  call functional%updateAdjointForcing(region, .false.)

  ! Add adjoint forcing to righthand side.
  do i = 1, size(state0)
    region%states(i)%rightHandSide = 0.0_wp
    region%states(i)%adjointForcingFactor = 1.0_wp
  end do
