...
def grid(numX,numY):
    ...
    for i in range(5):
        g.xyz[i][:,:,0,2] = 0.
    # center, N, W, S, E
    g.xyz[0][:,:,0,:2] = np.transpose(np.meshgrid(x0, y0))
    g.xyz[1][:,:,0,:2] = np.transpose(np.meshgrid(-x0, y0+Ly),[1,2,0])
    g.xyz[2][:,:,0,:2] = np.transpose(np.meshgrid(-Lx-x0, -y0))
    g.xyz[3][:,:,0,:2] = np.transpose(np.meshgrid(x0, -y0-Ly),[1,2,0])
    g.xyz[4][:,:,0,:2] = np.transpose(np.meshgrid(Lx+x0, y0))
