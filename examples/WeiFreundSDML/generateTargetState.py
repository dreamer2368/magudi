...
if __name__ == '__main__':
    ...
    slopeOfVorticityThickness = 0.05
    ...
    soln.Q[0][:,:,:,0] = 1.
    soln.Q[0][:,:,:,1] = lowerFluidVelocity + 0.5 * (upperFluidVelocity - lowerFluidVelocity) * (1. + np.tanh(2. * y / (1. + slopeOfVorticityThickness * np.where(x > 0., x, np.zeros_like(x)))))
