...
class InstabilityMode:
    ...
    def __init__(self, n=0, omega=0., theta=1., M1=0., M2=0.,
    ...
    def update(self):
        self.T1 = 1. / (1. + 0.5 * (self.gamma - 1.) *
                        self.M1 ** 2) / (self.gamma - 1.)
        self.T2 = 1. / (self.gamma - 1.)
        self.U1 = self.M1 * np.sqrt((self.gamma - 1.) * self.T1)
        self.U2 = self.M2 * np.sqrt((self.gamma - 1.) * self.T2)
        return self
    ...
    def mean_flow(self, r):
        ...
        mean_flow[0] = (self.U1 - self.U2) * u
        mean_flow[1] = 1. / (self.gamma - 1.) / ((
            self.T1 * (mean_flow[0] - self.U2) + self.T2 * (
                self.U1 - mean_flow[0])) / (self.U1 - self.U2) + 0.5 * (
                    mean_flow[0] - self.U2) * (self.U1 - mean_flow[0]))
        mean_flow[2] = np.sqrt(1. / mean_flow[1])
        mean_flow[3] = (self.U1 - self.U2) * u_dot
        mean_flow[4] = -(self.gamma - 1.) * mean_flow[1] ** 2 * ((
            self.T1 - self.T2) / (self.U1 - self.U2) + 0.5 * (
                self.U1 + self.U2) - mean_flow[0]) * mean_flow[3]
        return mean_flow
    ...
    def inner_solution(self, r, alpha):
        ...
        eta = np.sqrt((self.omega - alpha * (self.U1 - self.U2)) ** 2 /
                      (self.gamma - 1.) * self.T1 - alpha ** 2)
...
def target_state(g, M1=1.5, M2=0.2, gamma=1.4):
    s = p3d.Solution().copy_from(g).quiescent(gamma)
    T1 = 1. / (1. + 0.5 * (gamma - 1.) * M1 ** 2) / (gamma - 1.)
    T2 = 1. / (gamma - 1.)
    U1 = M1 * np.sqrt((gamma - 1.) * T1)
    U2 = M2 * np.sqrt((gamma - 1.) * T2)
    for i, xyz in enumerate(g.xyz):
        ...
        theta = np.select(condlist, [0.04 + np.zeros_like(z),
                                     0.46 * z / 24. + 0.04, 0.5])
        U_j = np.where(z <= 2.65, U1 + np.zeros_like(z),
                       (U1 - U2) * np.exp(-(z - 2.65) / 25.) + U2)
        T_j = np.where(z <= 0., T1 + np.zeros_like(z),
                        T2 / (1. + (T2 - T1) / T1 * np.exp(-0.078 * z)))
        for k in range(z.size):
            s.q[i][:,:,k,3] = 0.5 * (U_j[k] - U2) * (1. + np.tanh(
                0.25 / theta[k] * (r0[k] / r - r / r0[k]))) + U2
            s.q[i][:,:,k,0] = 1. / (gamma - 1.) / ((T_j[k] * (
                s.q[i][:,:,k,3] - U2) + T2 * (U_j[k] - s.q[i][:,:,k,3])) / (
                    U_j[k] - U2) + 0.5 * (s.q[i][:,:,k,3] - U2) * (
                        U_j[k] - s.q[i][:,:,k,3]))
...
def eigenmodes(M1=1.5, M2=0.2, theta_j=0.04, show_progress=True):
    u_j = InstabilityMode(M1=1.5, M2=0.2,
    ...
    for j, St_ in enumerate(St):
        ...
        for i, n_ in enumerate(n):
            modes[j][i] = InstabilityMode(
                M1=1.5, M2=0.2, theta=theta_j, n=n_,
...
def axisymmetric_grid(num_axial=512, r_max=12.5):
    z = axial_coordinate(num_axial=num_axial)
    r = np.linspace(0., r_max, 1001)
    x, y = np.meshgrid(z, r)
    g = p3d.Grid().set_size([r.size, 1, z.size], True)
    g.xyz[0][:,0,:,0] = y
    g.xyz[0][:,0,:,2] = x
    return g

if __name__ == '__main__':
    filename = 'MultiblockJetCoflow.xyz'
    if not os.path.isfile(filename):    
        g = grid(60, 196, 132, a_inner=0.24, p_inner=1.08634735266)
        g.save(filename)
    g = p3d.fromfile(filename)
    target_state(g).save('MultiblockJetCoflow.target.q')
    gi = extract_inflow(g)
    gi.save('MultiblockJetCoflow.inflow.xyz')
    modes = eigenmodes()
    for i, mode in enumerate(modes):
        sr, si = inflow_perturbations(gi, mode)
        sr.save('MultiblockJetCoflow-%02d.eigenmode_real.q' % (i + 1))
        si.save('MultiblockJetCoflow-%02d.eigenmode_imag.q' % (i + 1))
