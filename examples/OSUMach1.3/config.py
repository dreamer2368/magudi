...
def draw_circular_arc(f, label, p1, p2, c):
    print('gg::conBegin', file=f)
    print('gg::segBegin -type CIRCULAR_ARC', file=f)
    print('gg::segAddControlPt {%f %f %f}' % (p1[0], p1[1], p1[2]), file=f)
    print('gg::segAddControlPt {%f %f %f}' % (p2[0], p2[1], p2[2]), file=f)
    print('gg::segAddControlPt -alternate CENTER {%f %f %f}' % \
        (c[0], c[1], c[2]), file=f)
    print('gg::segEnd', file=f)
    print('set %s [gg::conEnd]' % (label), file=f)
    return None

def draw_line3d(f, label, p1, p2):
    print('gg::conBegin', file=f)
    print('gg::segBegin -type 3D_LINE', file=f)
    print('gg::segAddControlPt {%f %f %f}' % (p1[0], p1[1], p1[2]), file=f)
    print('gg::segAddControlPt {%f %f %f}' % (p2[0], p2[1], p2[2]), file=f)
    print('gg::segEnd', file=f)
    print('set %s [gg::conEnd]' % (label), file=f)
...
def rotate_copy_connector(f, label, p1, p2, angle, new_label=None,
                          replace=False):
    if replace is True:
        print('gg::conTransformBegin $%s -maintain_linkage' % (label), file=f)
    else:
        print('gg::conCopyBegin $%s' % (label), file=f)
    print('gg::xformRotate {%f %f %f} {%f %f %f} %f' % \
        (p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], angle), file=f)
    if replace is True:
        print('gg::conTransformEnd', file=f)
    else:
        print('set %s [gg::conCopyEnd]' % (new_label), file=f)
    return None

def rotate_copy_domain(f, label, p1, p2, angle, new_label=None,
                       replace=False):
    if replace is True:
        print('gg::domTransformBegin $%s -maintain_linkage' % (label), file=f)
    else:
        print('gg::domCopyBegin $%s' % (label), file=f)
    print('gg::xformRotate {%f %f %f} {%f %f %f} %f' % \
        (p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], angle), file=f)
    if replace is True:
        print('gg::domTransformEnd', file=f)
    else:
        print('set %s [gg::domCopyEnd]' % (new_label), file=f)
    return None

def rotate_copy_block(f, label, p1, p2, angle, new_label=None,
                      replace=False):
    if replace is True:
        print('gg::blkTransformBegin $%s -maintain_linkage' % (label), file=f)
    else:
        print('gg::blkCopyBegin $%s' % (label), file=f)
    print('gg::xformRotate {%f %f %f} {%f %f %f} %f' % \
        (p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], angle), file=f)
    if replace is True:
        print('gg::blkTransformEnd', file=f)
    else:
        print('set %s [gg::blkCopyEnd]' % (new_label), file=f)
    return None

def domain(f, edge1, edge2, edge3, edge4, label):
    print('gg::domBegin -type STRUCTURED', file=f)
    print('gg::edgeBegin', file=f)
    print('gg::edgeAddCon $%s' % (edge1), file=f)
    print('gg::edgeEnd', file=f)
    print('gg::edgeBegin', file=f)
    print('gg::edgeAddCon $%s' % (edge2), file=f)
    print('gg::edgeEnd', file=f)
    print('gg::edgeBegin', file=f)
    print('gg::edgeAddCon $%s' % (edge3), file=f)
    print('gg::edgeEnd', file=f)
    print('gg::edgeBegin', file=f)
    print('gg::edgeAddCon $%s' % (edge4), file=f)
    print('gg::edgeEnd', file=f)
    print('set %s [gg::domEnd]' % (label), file=f)
    return None
    ...
    with open(filename, 'w') as f:
        ...
        for theta_ in theta:
            ...
            print('gg::dbCurveAddPt {%.6f %.6f %.6f}' % \
                (r * np.cos(theta_), r * np.sin(theta_), 0.), file=f)
        print('set db_inner_block_E [gg::dbCurveEnd]', file=f)
        print("""
        ...
        print('gg::conDim [list $inner_block_E $inner_block_N ' \
            '$inner_block_W $inner_block_S] %i' % (num_azimuthal / 4 + 1), file=f)
        ...
        print('gg::conDim [list $tmp_extrusion_SE] %i' % (num_radial), file=f)
        print('gg::conSetBreakPt $tmp_extrusion_SE {%f %f 0}' % \
            ((r_se + num_const_dr * dr_interface) * np.cos(theta_se),
             (r_se + num_const_dr * dr_interface) * np.sin(theta_se)), file=f)
        print('gg::conSetBreakPt $tmp_extrusion_SE {%f %f 0}' % \
            (r_nozzle * np.cos(theta_se), r_nozzle * np.sin(theta_se)), file=f)
        print('gg::conSubConDim $tmp_extrusion_SE [list %i %i %i]' % \
            (num_const_dr + 1, num_radial_inside_nozzle - num_const_dr + 1,
             num_radial - num_radial_inside_nozzle), file=f)
        print('gg::conBeginSpacing $tmp_extrusion_SE -sub 3 %f' % (dr_min), file=f)
        print('gg::conBeginSpacing $tmp_extrusion_SE -sub 2 %f' % \
            (dr_interface), file=f)
        print('gg::conEndSpacing $tmp_extrusion_SE -sub 2 %f' % (dr_min), file=f)
        print('gg::conBeginSpacing $tmp_extrusion_SE -sub 1 %f' % \
            (dr_interface), file=f)
        print('gg::conEndSpacing $tmp_extrusion_SE -sub 1 %f' % \
            (dr_interface), file=f)
        if p_inner < 2.:
        ...
        """, file=f)
        print('gg::domExtrusionAtt -stop_height %f' % (r_outer), file=f)
        print('gg::domExtrusionStep %i' % (num_radial), file=f)
        print("""
        ...
        if num_axial > 1:
            ...
            print('gg::domTransformBegin [list $dom_inner $dom_outer_E ' \
                '$dom_outer_N $dom_outer_W $dom_outer_S] -maintain_linkage', file=f)
            print('gg::xformTranslate {0 0 %f}' % (z_min), file=f)
            print('gg::domTransformEnd', file=f)
            # Create the axis connector.
            draw_line3d(f, 'axis', [0., 0., z_min], [0., 0., z_max])
            print('gg::conDim [list $axis] %i' % (num_axial), file=f)
            print('gg::conSetBreakPt $axis {0 0 %f}' % \
                (potential_core_length), file=f)
            print('gg::conBreakPtSpacing $axis -breakpt 1 %f' % (dz_min), file=f)
            # Extrude blocks.
            print('gg::blkExtrusionBegin [list $dom_inner $dom_outer_E ' \
                '$dom_outer_N $dom_outer_W $dom_outer_S] -default PATH', file=f)
            print('gg::blkExtrusionAtt -path_connector ' \
                '[list [list $axis 1] [list $axis 2]]', file=f)
            print('gg::blkExtrusionStep %i' % (num_axial), file=f)
            print("""
            ...
            """, file=f)
            # Re-orient blocks.
            print('gg::blkSpecifyIJK [list $blk_outer_E $blk_outer_N ' \
                '$blk_outer_W $blk_outer_S] 5 4 1', file=f)
            print('gg::dbDelete ALL', file=f)
            # Save PLOT3D grid file.
            print('gg::blkExport ALL \"OSUMach1.3.xyz\" -style PLOT3D ' \
                '-form UNFORMATTED -precision DOUBLE -endian NATIVE', file=f)
        else:
            print('gg::dbDelete ALL', file=f)
            # Save PLOT3D grid file.
            print('gg::domExport ALL \"OSUMach1.3.xyz\" -style PLOT3D ' \
                '-form UNFORMATTED -precision DOUBLE -endian NATIVE', file=f)
